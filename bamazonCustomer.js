...
// only positive integers for the inputs
...
function promptUserPurchase () {
  ...
  inquirer.prompt([
    ...
    },
    {
      type: 'input',
      name: 'quantity',
      message: 'Item Quantity you need?',
      validate: InputVal,
      filter: Number
    }
  ]).then(function (input) {
    ...
    connection.query(queryStr, { item_id: item }, function (err, data) {
      ...
      if (data.length === 0) {
        ...
        if (quantity <= productData.stock_quantity) {
          console.log('YES!, the product is in stock! Your Order has been placed!')
...
// retrieves inventory from database and outputs to console
function showInventory () {
  // console.log('___ENTER showInventory___');

  // Construct the databaseb query string
